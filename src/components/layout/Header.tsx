  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 bg-background transition-all duration-200 ${
        scrolled ? 'border-b-[3px] border-foreground shadow-[0_3px_0_0_var(--foreground)]' : 'border-b-2 border-border'
      }`}
    ...
    >
      <div className="flex items-center h-14 px-4 max-w-7xl mx-auto">
        ...
        >
          <span
            className="flex items-center justify-center w-9 h-9 rounded-lg text-xs font-black border-2 border-foreground shadow-[2px_2px_0_0_var(--foreground)]"
            style={{ backgroundColor: '#FFD700', color: '#0A0A0A' }}
          >
            BB
          </span>
          <span className="text-xl font-heading font-black text-foreground">
            <span className="text-[#FFD700]">Bangla</span>{' '}
        ...
        <div className="flex items-center gap-2 ml-auto shrink-0">
          ...
          <button
            ...
            className="nb-btn-sm px-2 py-1 text-xs bg-card text-foreground"
          >
            {language === 'en' ? 'বাং' : 'EN'}
          </button>
          ...
          >
            ...
            {mounted && displayItemCount > 0 && (
              <span className="absolute -top-2 -right-2 nb-badge bg-[#EF4444] text-white text-[9px] min-w-[18px] text-center">
